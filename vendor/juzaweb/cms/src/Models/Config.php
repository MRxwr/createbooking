...
class Config extends Model
{
    ...
    public static function getConfig($key, $default = null,$company_id=1)
    {
       
        $value = Cache::rememberForever('jw_config.' . $key, function () use ($key, $default) {
            $config = Config::where('code', '=', $key)->where('company_id', '=', $company_id)->first(['value']);
    ...
    public static function setConfig($key, $value = null,$company_id=1)
    {
        if (is_array($value)) {
        ...
        $config = Config::firstOrNew(['code' => $key,'company_id' => $company_id]);
        $config->code = $key;
        $config->value = $value;
        $config->save();
